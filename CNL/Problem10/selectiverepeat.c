...
// 3. Accept the window size from the client(should be <=40) 4. Accept the packets from the network layer.
// 5. Calculate the total frames/windows required.
// 6. Send the details to the client(totalpackets,totalframes.) 7. Initialise the transmit buffer.
// 8. Built the frame/window depending on the windowsize.
// 9. Transmit the frame.
// 10. Wait for the acknowledgement frame.
// 11. Check for the acknowledgement of each packet and repeat the process for the packet for which the negative acknowledgement isreceived.
...
socklen_t len;
int windowsize,totalpackets,totalframes,framessend=0,i=0,j=0,k,l,m,n,repacket[40]; ack acknowledgement;
frame f1;
char req[50];
serversocket=socket(AF_INET,SOCK_DGRAM,0); bzero((char*)&serveraddr,sizeof(serveraddr)); serveraddr.sin_family=AF_INET; serveraddr.sin_port=htons(5018); serveraddr.sin_addr.s_addr=INADDR_ANY; bind(serversocket,(sockaddr*)&serveraddr,sizeof(serveraddr));
...
//starting the process of sending //initialising the transmit buffer.
